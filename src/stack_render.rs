        ...
        .map(|spans| {
            ...
            for span in spans {
                match span.style {
                    ...
                    Style::Indicator => {
                        // Indicators like @ and ✓ get green; others are plain
                        let text = &span.text;
...
/// Build the plain-text trunk merge string (for tests that assert on plain text).
fn build_trunk_merge_plain(num_cols: usize) -> String {
    if num_cols <= 1 {
        return "│".to_string();
    }
    let mut line = String::from("├─");
...
fn build_node_content(row: &DisplayRow) -> Vec<Span> {
    ...
    // Indicators
    if !row.indicator_str.is_empty() {
        spans.push(Span::plain(" "));
        spans.push(Span::new(&row.indicator_str, Style::Indicator));
    }
...
mod tests {
    ...
    fn trunk_merge_single_column() {
        assert_eq!(build_trunk_merge_plain(1), "│");
    }

    #[test]
    fn trunk_merge_two_columns() {
        assert_eq!(build_trunk_merge_plain(2), "├─╯");
    }

    #[test]
    fn trunk_merge_three_columns() {
        assert_eq!(build_trunk_merge_plain(3), "├─┴─╯");
    }

    #[test]
    fn trunk_merge_four_columns() {
        assert_eq!(build_trunk_merge_plain(4), "├─┴─┴─╯");
    }
