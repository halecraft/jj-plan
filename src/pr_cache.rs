...
//!
//! The cache is stored in `.jj/repo/jj-plan/pr-cache.toml` and can be safely
...
pub fn load_pr_cache(workspace_root: &Path) -> Result<PrCache> {
    ...
    let cache: PrCache = toml::from_str(&content)
        .map_err(|e| JjPlanError::Config(format!("failed to parse {}: {e}", path.display())))?;
...
pub fn save_pr_cache(workspace_root: &Path, cache: &PrCache) -> Result<()> {
    ...
    let content = toml::to_string_pretty(&cache_to_save)
        .map_err(|e| JjPlanError::Config(format!("failed to serialize PR cache: {e}")))?;
...
mod tests {
    ...
    fn test_pr_cache_path() {
        ...
        assert!(path.ends_with(".jj/repo/jj-plan/pr-cache.toml"));
    }
