...
Read-only jj commands pass through via `exec`. `diff`/`interdiff` and the rest are zero-overhead (they surface trees, not descriptions). The description-surfacing reads — `log`, `show`, `evolog` — go through a **drift gate**: a near-zero-cost content-hash check that flushes pending plan-file edits (and opens jj-lib) only when the plan files have actually changed since the last sync, so they never render a stale description. Mutating commands go through the wrap lifecycle: flush pending edits → run command → reload → sync plan files → show stack.
...
All display surfaces show **tip (working copy, furthest from trunk) at top** and **trunk-nearest at bottom**, matching `jj log` convention. This applies to:

- `jj stack` terminal visualization
- `jj plan` summary stack listing
- `jj plan` orientation message (when `@` is not on a plan)
- PR stack comments (row 1 = tip)
- Plan file filesystem sort order (letter prefix ensures tip sorts first in `ls`)
- `stack.md`

The internal data model (`stack.segments`) remains trunk-first (index 0 = trunk-nearest). This is the natural output of jj-lib's topological revset evaluation and is required by the submit and merge engines for correct base-branch chaining. Display reversal is a presentation concern handled at render time.
...
```
  ◉ kpqxywon (@, ~) feat-api | Add API endpoints → .jj-plan/a-03-feat-api.md
  ○ mtzrlpvq (~) feat-session | Implement session management → .jj-plan/b-02-feat-session.md
  ○ lonpswlw (✓) feat-auth | Extract auth module → .jj-plan/c-01-feat-auth.md
  ◆ trunk()
```
...
```
  ◉ kpqxywon feat-api → .jj-plan/a-03-feat-api.md
  │ (@, ~) Add API endpoints
  │
  ○ mtzrlpvq feat-session → .jj-plan/b-02-feat-session.md
  │ (~) Implement session management
  │
  ○ lonpswlw feat-auth → .jj-plan/c-01-feat-auth.md
  │ (✓) Extract auth module
  │
  ◆ trunk()
```
...
- `→ .jj-plan/L-NN-bookmark.md` = plan file relative path (terminal only, gated by `RenderOptions::show_paths`).
...
<!-- generated by jj-plan — do not edit -->
  ...
  ◉ kpqxywon [feat-api](./a-03-feat-api.md)
  │ (@, ~) Add API endpoints
  │
  ○ mtzrlpvq [feat-session](./b-02-feat-session.md)
  │ (~) Implement session management
  │
  ○ lonpswlw [feat-auth](./c-01-feat-auth.md)
  │ (✓) Extract auth module
  │
  ◆ trunk()
...
The `StackColumn` struct includes a `plan_dir_name` field (e.g. `".jj-plan"`) so the renderer can compose full relative paths at render time from `row.plan_filename`. For example, if `plan_dir_name` is `".jj-plan"` and `row.plan_filename` is `"a-03-feat-api.md"`, the renderer produces `→ .jj-plan/a-03-feat-api.md`. This keeps path construction out of callers and centralizes it in the render layer.

---

## Read-path drift gate (`src/sync_state.rs`)

jj-plan can't hook the editor or an LLM's file-write, so a direct `.jj-plan/*.md` edit only reaches the jj description on the next command that flushes. Read-only commands that **render a description** — `log`, `show`, `evolog` — historically `exec`'d real `jj` directly and could therefore show a stale description until some later command flushed. The drift gate closes that loop without giving up the fast path.
...
**Gotcha:** a drifted `jj log` now produces a `describe` operation (the flush). This is the same coherence cost `status` and mutating commands already pay; `jj op log` may show a `describe` op authored by what looked like a read.
...
Plan files are named `L-NN-BOOKMARKNAME.md`:

- `L` = single lowercase letter filesystem sort key. Tip (furthest from trunk) gets `'a'` (sorts first in `ls`), each step toward trunk increments: `'b'`, `'c'`, etc. Clamped at `'z'` for positions ≥ 25. This ensures `ls .jj-plan/` shows tip first, matching `jj stack` display order.
- `NN` = zero-padded dependency index (01 = trunk-nearest, first to merge). This carries semantic meaning and matches `jj plan go <N>`.
- `BOOKMARKNAME` = bookmark name with `/` encoded as `--`.

Examples (3-plan stack):
- `feat-api` (tip)            → `a-03-feat-api.md`
- `feat-session` (middle)     → `b-02-feat-session.md`
- `feat-auth` (trunk-nearest) → `c-01-feat-auth.md`

The dual-purpose encoding separates two concerns: the letter prefix controls filesystem sort order (presentation), while the numeric index carries dependency-chain meaning (data model). `jj plan go 1` navigates to trunk-nearest (`01`), regardless of display order.

### Registry-authoritative resolution

Filenames are **never decoded** back to bookmark names via string replacement. Instead, when reading the plan directory, each `L-NN-ENCODED.md` file is matched against the `PlanRegistry` to find the canonical bookmark name:

1. Parse `L-NN-ENCODED.md` to extract the encoded portion.
...
`parse_plan_filename()` also accepts the legacy `NN-ENCODED.md` format (no letter prefix) for backward compatibility. The sync pipeline renames these to the new format on the next operation.
...
- **`generate_stack_comment(chain, current_bookmark, default_branch)`**: Produces a markdown table showing all PRs in the stack with the current PR highlighted in bold with a 👈 indicator. The chain is received in trunk-to-tip order (from the call site) and reversed internally so the tip is displayed first, matching the display direction convention. Row ordinals use the dependency index (`01` = trunk-nearest, matching `jj plan go <N>` and file numbering). A `◆ {default_branch}` row is appended at the bottom to show the stack's base.
